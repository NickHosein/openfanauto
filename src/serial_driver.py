...
class SerialHardware(object):
    def __init__( self, port_info, flush_on_write=True, serialPrefix = '', serialSuffix = '\r\n', timeout=0.2, debug_uart=False):
        ...
        if isinstance(port_info, str):
            self.port_api = self._find_port_by_name(port_info)
            if self.port_api:
                self.port_info = self.port_api
            else:
                raise Exception(f"Could not find COM port: {port_info}")
        else:
    ...
    def description(self):
        int_error = 0 # Just to ensure something is here
    ...
    def _find_port_by_name(self, port_info):
        ...
        for port in availablePorts:
            ...
            if port.device == port_info:
                logger.debug("Using '{}' as Fan Controller COM port".format(port.device))
                return port
        return None
    ...
    def _read_until_re_match(self, status_re=None, timeout=2):
        ...
        if not status_re:
            regex = status_re
        compiled_re = re.compile(regex, flags=re.IGNORECASE)    #pylint: disable=possibly-used-before_assignment
    ...
    def serial_transaction(self, payload, ignore_response=False):
        ...
        try:
            ...
            logger.debug("serial_transaction sending: %s", payload)
